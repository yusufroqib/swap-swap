import { ChainId, Currency, Ether, NativeCurrency, Token, UNI_ADDRESSES, WETH9 } from '@zentraswap/sdk-core'
...
export const WRAPPED_NATIVE_CURRENCY: { [chainId: number]: Token | undefined } = {
  ...
  [ChainId.PHAROS_DEVNET]: new Token(
    ChainId.PHAROS_DEVNET,
    '0x147fC266f80CC58fCb91F86c8f3864972D52D627',
    18,
    'WPTT',
    'Wrapped Pharos Test Token'
  ),
}
...
export function isPharosDevnet(chainId: number): chainId is ChainId.PHAROS_DEVNET {
  return chainId === ChainId.PHAROS_DEVNET
}

class PharosDevnetNativeCurrency extends NativeCurrency {
  equals(other: Currency): boolean {
    return other.isNative && other.chainId === this.chainId
  }

  get wrapped(): Token {
    if (!isPharosDevnet(this.chainId)) throw new Error('Not Pharos Devnet')
    const wrapped = WRAPPED_NATIVE_CURRENCY[this.chainId]
    invariant(wrapped instanceof Token)
    return wrapped
  }
  ...
  public constructor(chainId: number) {
    if (!isPharosDevnet(chainId)) throw new Error('Not Not Pharos Devnet')
    super(chainId, 18, 'PTT', 'PTT')
  }
}
...
export function nativeOnChain(chainId: number): NativeCurrency | Token {
  ...
  } else if (isPharosDevnet(chainId)) {
    nativeCurrency = new PharosDevnetNativeCurrency(chainId)
  } else {
